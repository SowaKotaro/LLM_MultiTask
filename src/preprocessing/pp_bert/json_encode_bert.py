def make_bioseq(json,cls,sep,arg_role,frame_point):
...
def make_target_seq(json,cls,sep,params):
    return []
...
def encode(data,params):
    ...
    for i,d in enumerate(data):
        ...
        if params["bio"]:
            target_seq,arg_tags = make_bioseq(d,len(cls),len(sep),params["arg_role"],params["not_O_fidtag"]) 
...
def syntax_encode(syntax_json,use_parents_node,use_child_node): # 自己ループはtorchのGCNConvのパラメータで設定 use_parents_node use_child_nodeが両方ともTrueで無向(双方向有向)グラフ
    tag2id = {}
    id_count = 0
    new_syntax_dic = {}
...
def encode_multi(data,params):
    ...
    for i,d in enumerate(data): # 全てのデータセットについて学習に必要なものを取り出す
        if params["with_predicate"]: # True  # 文章のあとに述語を連結する cls 文章 sep 述語 sep　形式
            ...
            input.append(cls + d["sentence"].split() + sep)
        if params["bio"]: # True 
            target_seq,arg_tags = make_bioseq(d,len(cls),len(sep),params["arg_role"],params["not_O_fidtag"]) # BIO形式の出力を作成
